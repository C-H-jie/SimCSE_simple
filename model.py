...
class SimcseModel(nn.Module):
    ...
    def __init__(self, pretrained_model, pooling, dropout=0.3,neutral_dropout=0.5):
...
def RCL_unsup_rank_loss_ClE(y_pred,y_pred_CLN,sim, device , temp=0.05):
    ...
    疑问：
        引入中性样本根本目的是为了让负例之间距离更大，正例之间距离更小
        但是这里的p3的计算方式是不是有问题，因为p3的计算方式是将所有的负例之间的距离都加起来了
        这样的话，如果负例之间的距离很大，那么p3就会很大，这样的话，p1/p2+p3就会很小
        这样的话，loss就会很大。为了减小loss，就会让负例之间的距离更小，这样就和引入中性样本的目的相悖了
    '''
