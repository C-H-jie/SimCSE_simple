...
def RCL_unsup_rank_loss2(y_pred, device, temp=0.05):
    ...
    # 根据条件计算损失
    # 正例-正例 = 0  
    # 负例-正例 若小于0说明正例的相似度大于负例的相似度 此时loss = 0
    # 若大于0说明正例的相似度小于负例的相似度 此时loss = e^(正例-负例)
    # 由于losss = log∑∑e^lpart 故而将小于0的值设置为一个极小的值，这样在求logsumexp时会被忽略
    lpair_components = torch.where(result <= 0, torch.tensor(-1.2000e+13, device=device), result)
...
if __name__ == "__main__":
    ...
    # 示例矩阵
    matrix = torch.randn(128, 768).to(device)
    cos_similarities = RCL_unsup_rank_loss(matrix,device)
    print(cos_similarities)

    cos_similarities = RCL_unsup_rank_loss2(matrix,device)
    print(cos_similarities)
