...
from model import SimcseModel, simcse_unsup_loss, simcse_sup_loss, RCL_unsup_rank_loss2,RCL_unsup_rank_loss_ClE3,RCL_unsup_rank_loss_ClE
...
def train(model, train_loader, dev_loader,optimizer, args):
    ...
    for epoch in range(args.epochs):
        for batch_idx, data in enumerate(tqdm(train_loader,total=len(train_loader))):
            ...
            input_ids_neutral = input_ids[::2]
            attention_mask_neutral = attention_mask[::2]
            token_type_ids_neutral = token_type_ids[::2]

            out = model(input_ids, attention_mask, token_type_ids)
            # out = model.dropout1(out)
            out_neutral = model(input_ids_neutral, attention_mask_neutral, token_type_ids_neutral,is_neutral=True)
            # out_neutral = model.dropout(out_neutral)

            if args.train_mode == 'unsupervise':


                loss,sim = simcse_unsup_loss(out, device)
                # loss2 = RCL_unsup_rank_loss2(out, device)
                # loss,sim = RCL_unsup_rank_loss2(out, device)
                # out_neutral = out[::2]
                loss = loss + RCL_unsup_rank_loss_ClE3(out,out_neutral,sim, device)*args.coefficient
                # print(loss)
...
def main(args):
    ...
    model = SimcseModel(pretrained_model=args.pretrain_model_path, pooling=args.pooler, dropout=args.dropout , neutral_dropout=args.neutral_dropout ).to(
        args.device)
    if args.do_train:
        # 加载数据集
    ...
    if args.do_predict:
        ...
        if corrcoef >76.85:
            logger.info('testset corrcoef:{} is higher than SimCSE in STSB'.format(corrcoef))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--neutral_dropout", type=float, default=0.84)
    parser.add_argument("--coefficient", type=float, default=0.11)
    ...
    args.output_path = join(args.output_path, args.train_mode, 'RCL3-{}-lr-{}-dropout-{}-neutral_dropout{}'.format(args.batch_size_train, args.lr, args.dropout,args.neutral_dropout))
